...
const petalGeometry = new THREE.SphereGeometry(2.4, 6, 4);

export function Tuft(tileIndex, color) {
  const tuft = new THREE.Group();
  tuft.position.x = tileIndex * WORLD.TILE_SIZE;

  const material = flatMaterial({ color, roughness: 0.85 });
  for (let i = 0; i < COUNT; i++) {
    ...
    if (Math.random() < FLOWER_CHANCE) {
      const petal = new THREE.Mesh(
        petalGeometry,
        flatMaterial({ color: FLOWER_COLORS[Math.floor(Math.random() * FLOWER_COLORS.length)], roughness: 0.6 }),
      );
